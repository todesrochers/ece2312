...
%PART 3: LOWPASS AND DOWNSAMPLE 
    ...
    sampling_freq = 44100; %have to keep this same so we get 5 sec
...
%REASSEMBLE
  %PART 5:
    ...
    xlabel('Time (s)'); 
    ylabel('Frequency (Hz)');
    title('Reassembled Brown Fox versus Time');
    
    sound(fs1, sampling_freq);
    
   %saving to WAV file
   audiowrite("Team3-synthesized.wav", fs1, sampling_freq);
   clear fs1 sampling_freq;
   audioinfo("Team3-synthesized.wav")
   [fs1,sampling_freq] = audioread('Team3-synthesized.wav');
end
